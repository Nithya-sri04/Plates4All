...
<canvas id="myChart" style="width:100%;max-width:600px"></canvas> <br> <canvas id="donateChart" style="width:100%;max-width:600px"></canvas>
...
<script>
      <?php
            ...
            $female=$ro2['count'];
            $male=$row['count'];
           // Fetch location-wise donation counts from the database
...
while ($row = mysqli_fetch_assoc($result)) {
    $locations[] = ucwords($row['location']); // Capitalize names
    $counts[] = $row['count'];
}
?>
...
var bar=["#06C167","blue","red"]
 ...
 var xplace = <?php echo json_encode($locations); ?>;
  var yplace = <?php echo json_encode($counts); ?>;

  // Generate random colors for bars
  var barColors = xplace.map(() => "#" + Math.floor(Math.random()*16777215).toString(16));
  ...
  new Chart("donateChart", {
    ...
    options: {
      ...
      title: {
        display: true,
        text: "Food Donation Details by Location"
      },
      responsive: true,
      scales: {
        y: {
